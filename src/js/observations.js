...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    newObject.id = observation.id;
    newObject.uri = observation.uri;
    newObject.coords[0] = observation.geojson.coordinates[0];
    newObject.coords[1] = observation.geojson.coordinates[1];
    newObject.date = new Date(observation.created_at);
    if (observation.taxon.preferred_common_name) {
      newObject.name = titleCase(observation.taxon.preferred_common_name);
    } else {
      newObject.name = titleCase(observation.taxon.name);
    }
    newObject.photoUrl = observation.taxon.default_photo.square_url;
    newObject.wikipediaUrl = observation.taxon.wikipedia_url;
    newObject.isNative = observation.taxon.native;
    newObject.isIntroduced = observation.taxon.introduced;
    if (observation.taxon.endangered == null) {
      newObject.isEndangered = false;
    } else {
      newObject.isEndangered = observation.taxon.endangered;
    }
    newObject.isThreatened = observation.taxon.threatened;
    return newObject;
  });
}
